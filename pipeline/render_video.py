...
Usage: python3 pipeline/render_video.py data/sermons/2026-08-30.json [--voice Kore] [--no-tts]
...
def wrap(draw, text, fnt, max_w, cjk):
    """Greedy wrap: per character for Chinese, per word for Korean/Latin."""
    lines, cur = [], ""
    ...
    if cur:
        lines.append(cur)
    # avoid a dangling punctuation mark on its own line
    fixed = []
    for l in lines:
        if fixed and cjk and l and l[0] in "，。！？；：、”）" and len(l) <= 2:
            fixed[-1] += l
        else:
            fixed.append(l)
    return fixed

def draw_block(draw, text, fnt, x, y, max_w, fill, cjk, spacing=1.25, align="center"):
...
def card_text(sermon, beat, progress, label, number=None):
    ...
    zh, ko, py = beat["zh"], beat["ko"], clean_pinyin(beat.get("pinyin", ""))
    size = 92 if len(zh) <= 14 else 78 if len(zh) <= 22 else 66
    def paint(dd, y0):
        y = y0
        if number is not None:
            f = font(SERIF_B, 150, SC)
            dd.text((W / 2 - dd.textlength(number, font=f) / 2, y), number, font=f, fill=GOLD); y += 210
        y = draw_block(dd, zh, font(SANS_B, size, SC), 90, y, W - 180, IVORY, cjk=True, spacing=1.3)
        y = draw_block(dd, py, font(SANS_R, 36, SC), 90, y + 24, W - 180, GOLD, cjk=False, spacing=1.3)
        y += 60
        dd.line((W / 2 - 60, y, W / 2 + 60, y), fill=GOLD, width=3)
        return draw_block(dd, ko, font(SANS_R, 46, KR), 110, y + 40, W - 220, MUTED, cjk=False, spacing=1.35)
...
def card_title(sermon, progress):
    ...
    y = draw_block(d, sermon["title"]["zh"], font(SERIF_B, 96, SC), 90, 700, W - 180, IVORY, cjk=True, spacing=1.25)
    y = draw_block(d, sermon["title"]["ko"], font(SANS_R, 52, KR), 90, y + 40, W - 180, MUTED, cjk=False)
...
def card_vocab(sermon, w, idx, progress):
    ...
    def paint(dd, y0):
        ...
        y = y0 + f.size + 60
        py = clean_pinyin(w["pinyin"]); f2 = font(SANS_R, 60, SC)
        dd.text((W / 2 - dd.textlength(py, font=f2) / 2, y), py, font=f2, fill=GOLD); y += 110
        y = draw_block(dd, w["ko"], font(SANS_B, 66, KR), 90, y, W - 180, ACCENT2, cjk=False)
        meta = f"{w.get('pos', '')}  ·  HSK {w.get('hsk', '')}"; f4 = font(SANS_R, 34, SC)
        dd.text((W / 2 - dd.textlength(meta, font=f4) / 2, y + 16), meta, font=f4, fill=MUTED)
        y += 120
        dd.line((W / 2 - 60, y, W / 2 + 60, y), fill=GOLD, width=3)
        y = draw_block(dd, ex, font(SANS_R, 46, SC), 100, y + 40, W - 200, IVORY, cjk=True, spacing=1.35)
        return draw_block(dd, ex_ko, font(SANS_R, 38, KR), 110, y + 24, W - 220, MUTED, cjk=False, spacing=1.35)
    y_end = paint(ImageDraw.Draw(Image.new("RGB", (W, H))), 0)
    paint(d, max(300, int(CENTER_Y - y_end / 2)))
    return img

def build_beats(sermon):
    beats = []
    ...
    beats.append({"kind": "title", "zh": sermon["title"]["zh"], "ko": sermon["title"]["ko"],
                  "tts": f"本周讲道：{sermon['title']['zh']}。经文：{sermon['scripture']['ref_zh']}。"})
    ...
    for i, w in enumerate(vocab, 1):
        ex = w.get("example_zh", "").replace("...", "").strip("…. ")
        beats.append({"kind": "vocab", "word": w, "idx": i, "zh": w["zh"], "ko": w["ko"], "pinyin": w["pinyin"],
...
def render(sermon_path, voice="Kore", use_tts=True, out_dir=None):
    sermon = json.load(open(sermon_path, encoding="utf-8"))
    sid = sermon["id"]
    out_dir = out_dir or os.path.join(ROOT, "data", "videos")
    os.makedirs(out_dir, exist_ok=True)
    beats = build_beats(sermon)
    tmp = tempfile.mkdtemp(prefix=f"cp-{sid}-")
    style = "请用温暖、清晰、稍慢的语速朗读下面的中文，只朗读文本本身"
    clips, cues, t = [], [], 0.0
    for i, b in enumerate(beats):
        ...
        if b["kind"] == "title":
            ...
            img = card_vocab(sermon, b["word"], b["idx"], progress)
        else:
        ...
        text = b.get("tts") or b["zh"]
        if use_tts:
            gemini.tts(text, voice=voice, out_wav=wav, style=style)
            dur = wav_seconds(wav)
        else:
    ...
    Image.open(os.path.join(tmp, "01.png")).convert("RGB").resize((540, 960)).save(os.path.join(out_dir, f"{sid}.poster.jpg"), quality=82)
    with open(os.path.join(out_dir, f"{sid}.vtt"), "w", encoding="utf-8") as f:
        f.write("WEBVTT\n\n")
        for c in cues:
            f.write(f"{fmt_ts(c['start'], True)} --> {fmt_ts(c['end'], True)}\n{c['zh']}\n{c['ko']}\n\n")
    with open(os.path.join(out_dir, f"{sid}.srt"), "w", encoding="utf-8") as f:
        for n, c in enumerate(cues, 1):
            f.write(f"{n}\n{fmt_ts(c['start'])} --> {fmt_ts(c['end'])}\n{c['zh']}\n{c['ko']}\n\n")
    json.dump({"id": sid, "duration": round(t, 2), "voice": voice, "tts_model": gemini.TTS_MODEL, "cues": cues},
              open(os.path.join(out_dir, f"{sid}.script.json"), "w", encoding="utf-8"), ensure_ascii=False, indent=1)
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    render(a.sermon, a.voice, not a.no_tts, a.out_dir)
